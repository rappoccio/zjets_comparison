#!/usr/bin/env bash
# Local interactive test of the aMC@NLO + HERWIG7 chain — same as
# gen_amcnlo_herwig_job.sh but local, verbose, keeps the run dir.
#   ./madgraph/run_local_herwig.sh [NEV] [SEED]      # defaults: 1000 events, seed 1
#
# NB: the NLO process compiles Fortran first (a few minutes, CPU-heavy); on lxplus
# prefer an interactive batch slot (condor_submit -i) over a busy login node.
set -euo pipefail
...
WORK="$PKG/madgraph/local_run_herwig"; rm -rf "$WORK"; mkdir -p "$WORK"; cd "$WORK"
sed -e "s/@SEED@/$SEED/g" -e "s/@NEV@/$NEV/g" "$PKG/madgraph/zjets_herwig.mg5" > card.mg5
echo "=== card.mg5 ==="; cat card.mg5; echo "================"

echo ">>> mg5_aMC card.mg5   (writes the HERWIG-counterterm LHE; its shower is a no-op here)"
